...
import { changeQty, removeFromCart, clearCart } from "../../store/slices/userSlice";
        ...
        <div className={`orderPageNew__list ${errors.cart ? "is-invalid" : ""}`}>
          {!items?.length ? (
            <div className="orderPageNew__empty">
              Your cart is empty.
              {errors.cart && (
                <div className="orderPageNew__hint">Add at least one item from Menu.</div>
              )}
            </div>
          ) : (
            items.map((it) => (
              <div className="orderItem" key={it.id}>
                <img className="orderItem__img" src={it.img} alt={it.meal} />

                <div className="orderItem__info">
                  <div className="orderItem__name">{it.meal}</div>
                </div>
